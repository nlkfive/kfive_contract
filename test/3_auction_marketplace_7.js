...
var nlgst, kfive, auctionMarketplace, storage, new_storage;
...
contract("Auction Marketplace", (accounts) => {
    ...
    describe('AUCTION 7: Change storage', async () => {
        var auctionId;
        ...
        it('(Account1) Create new auction NFT(7) successfully', async () => {
            const i = {
                nftID: 7,
                nftURI: "7",
        ...
        it('(Account2) Bid: 2 KFIVE; Deposit: 3 KFIVE; Fake: false', async () => {
            const i = {
            ...
            const o = {
                kfive_account2_balance: (10 - 3) * (10 ** tokenDecimals),
            }
        ...
        it('(Root) Create new storage', async () => {
            new_storage = await STORAGE.new( {from: root} );
        });

        it('(Root) Update storage address for marketplace', async () => {
            await auctionMarketplace.updateStorageAddress(new_storage.address, {
                from: root
            });

            const newStorage = await auctionMarketplace.marketplaceStorage();
            eq(new_storage.address, newStorage);
        });

        it('(Root) Update storage address for NLGST', async () => {
            await nlgst.updateStorageAddress(new_storage.address, {
                from: root
            });

            const newStorage = await nlgst.marketplaceStorage();
            eq(new_storage.address, newStorage);
        });
        ...
        it('(Root) Update OrderMarketplace in the Storage', async () => {
            ...
            await new_storage.updateAuctionMarketplace(i.auctionMarketplaceAddress, {
                from: root
            });
        });

        it('Transfer NFT7. Cannot because NFT7 is on auction', async () => {
            const i = {
                tokenId: 7,
                from: account1,
                to: account8
            ...
            const nft7_owner = await nlgst.ownerOf(7, { from: root })

            eq(account1, nft7_owner);
        });
        ...
        it('(Account2) reveal', async () => {
            const i = {
                nftAddress: nlgst.address,
                nftID: 7,
                value_account2: 2 * (10 ** tokenDecimals),
                fakeFalse: false,
                secret: web3.utils.fromAscii("1234"),
            }
            ...
            eq(account2_balance.toString(), 8 * (10 ** tokenDecimals));
        });
        ...
        it('AuctionEnd. Account1 call acutionEnd', async () => {
            await auctionMarketplace.auctionEnd(nlgst.address, 7, auctionId, { from: account1 });
        });

        it('Account2 will be the owner of NFT(7)', async () => {
            const o = {
                nft7_owner: account2,
            }

            const nft7_owner = await nlgst.ownerOf(7, { from: root })
            eq(o.nft7_owner, nft7_owner);
        });
    });
});