...
contract("Public Auction Marketplace", (accounts) => {
    ...
    describe('BLIND AUCTION 1: Scenario' + '\n' +
             ...
             '          + Account2 reveal' + '\n' +
             '          + Account5 reveal' + '\n' +
             '          + Account2 withdraw. Receive all deposit KFIVE' + '\n' +
             '          + Account5 withdraw -> receive reward', async () => {
        var auctionId;
        ...
        it('(Account2) Reveal. Cannot, before biddingEnd', async () => {
            const i = {
                bidValue: 6 * (10 ** tokenDecimals)
            }

            await u.assertRevert(auctionMarketplace.reveal(auctionId, nftAsset, secret, [i.bidValue], {
                from: account2
            }));
        ...
        it('(Root) Cancel auction. Cannot, only before biddingEnd', async () => {
            await u.assertRevert(auctionMarketplace.cancelAuction(nftAsset, auctionId, {
                from: root
            }));
        });
        ...
        it('(Account5) Reveal', async () => {
            const i = {
                bidValue1: 1 * (10 ** tokenDecimals),
                bidValue2: 4 * (10 ** tokenDecimals),
            }
        ...
        it('(Account2) Withdraw', async () => {
            ...
            let old_owner = await nlgst.ownerOf(1, {from: root});
            eq(account1, old_owner);
