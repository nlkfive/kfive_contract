    ...
    describe('PUBLIC AUCTION 1: Scenario' + '\n' +
        ...
        it('(Account1) Create new auction NFT(1) successfully', async () => {
            const i = {
                ...
                biddingEnd: Math.floor(new Date().getTime() / 1000 + 1000),
        ...
        it('(Account2) Cancel Auction. Cannot, only owner || CANCEL_ROLE', async () => {
        ...
        it('(Root) Grant CANCEL_ROLE to canceller', async () => {
            await auctionMarketplace.grantRole("0x" + keccak256("CANCEL_ROLE"), canceller, {
                from: root
            });
        });

        it('(Canceller) Cancel Auction', async () => {
