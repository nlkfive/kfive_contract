    ...
    IERC721Receiver
{
    ...
    {
        Race memory _race = _raceList.getRace(raceId);
        address[] memory results = new address[](_race.slots);
        if (randomSelected[raceId] != 0){
            for (uint256 slotId = 0; slotId < _race.slots; slotId++) {
        ...
        returns (address)
    {
        ...
        uint256 selectedIndex = randomSelected[raceId] % registrationList[raceId][slotId].length;
    ...
    {
        Race memory _race = _raceList.getRace(raceId);
        uint256[] memory results = new uint256[](_race.slots);
        for (uint256 slotId = 0; slotId < _race.slots; slotId++) {
